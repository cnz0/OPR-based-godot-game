...
var game_mode = GameMode.PLAYER_VS_PLAYER

var highlighted_tiles = []  # Lista pozycji podświetlonych pól
...
var original_tiles = {}  # Słownik przechowujący oryginalne typy pól
...
func _ready():
	# Kod generowania mapy i przeszkód
	
	if Global.game_mode == "PvP":
		game_mode = GameMode.PLAYER_VS_PLAYER
	elif Global.game_mode == "PvAI":
...
func is_obstacle(tile_position: Vector2i) -> bool:
	if get_cell_source_id(tile_position) == -1:
		return false  # Brak kafelka - nie jest przeszkodą
	
	var atlas_coords = get_cell_atlas_coords(tile_position)
	return atlas_coords == Vector2i(4, 0)  # Zmień na odpowiednie współrzędne przeszkody
...
func _on_unit_clicked(event: InputEvent, unit: CharacterBody2D) -> void:
	# Wyświetl tooltip dla jednostki
	...
	# Obsługa ruchu i walki po zakończeniu rozmieszczania
	if deployment_complete:
		# Jeśli nie ma wybranej jednostki
		if selected_unit == null:
			# Wybierz jednostkę gracza, jeśli ma punkty ruchu
			if unit.get_meta("Player") == current_player and unit_data["Movement"] != 0:
				print("Unit selected")
				selected_unit = unit
				show_movement_options(selected_unit)
				highlight_movement_options(selected_unit)  # Podświetl możliwe pola ruchu
			else:
				print("Unit has no move points left or it's not its turn")
		else:
			# Jeśli kliknięto na wrogą jednostkę, sprawdź sąsiedztwo i zaatakuj
			if unit.get_meta("Player") != current_player and is_adjacent(selected_unit, unit):
				print("Attacking adjacent enemy unit!")
				initiate_combat(selected_unit, unit)
				return  # Po walce nie wykonuj dalszych akcji
			# Jeśli kliknięto ponownie wybraną jednostkę, odznacz ją
				...
				print("Enemy unit is not adjacent or no unit selected.")
				
func is_adjacent(unit1: CharacterBody2D, unit2: CharacterBody2D) -> bool:
	var position1 = local_to_map(unit1.position)
	var position2 = local_to_map(unit2.position)

	# Lista przesunięć dla sąsiednich kafelków
	var offsets = [
	...
	# Sprawdź, czy jedna z pozycji sąsiaduje z drugą
	for offset in offsets:
		if position1 + offset == position2:
			return true
	return false
...
func dual_dice():
	...
	# Jeśli AI jest pierwsze, od razu wykonuje ruch
	if Global.game_mode == "PvAI" and current_player == 2:
...
func move_unit(target_tile: Vector2i):
	...
	# Odśwież podświetlenie
	highlight_movement_options(selected_unit)
...
func check_for_combat(unit: CharacterBody2D) -> Array:
	...
	for offset in offsets:
		...
		if adjacent_unit != null and adjacent_unit.get_meta("Player") != unit.get_meta("Player"):
			potential_targets.append(adjacent_unit)
...
func check_and_handle_end_of_turn():
	if game_mode == GameMode.PLAYER_VS_PLAYER:
		# Tryb gracz vs gracz - zachowuje obecne działanie
		current_player = 3 - current_player  # Zmiana gracza
		update_background_color()
		if all_units_moved(current_player):
			current_player = 3 - current_player
			update_background_color()
		if all_units_moved(current_player) and all_units_moved(3 - current_player):
			handle_end_of_round()

	elif game_mode == GameMode.PLAYER_VS_AI:
		# Tryb gracz vs AI
		if current_player == 1:  # Ruch gracza
			current_player = 3 - current_player
			update_background_color()
			if all_units_moved(current_player):  # Sprawdź ruch AI
				ai_controller.make_decision()  # AI podejmuje decyzję
			if all_units_moved(current_player) and all_units_moved(3 - current_player):
				handle_end_of_round()

		elif current_player == 2:  # Ruch AI
			ai_controller.make_decision()  # AI podejmuje decyzję
			current_player = 3 - current_player
			update_background_color()
			if all_units_moved(current_player) and all_units_moved(3 - current_player):
...
var f_score = {}  # Zmienna globalna dla `f_score`

func get_simple_path(start: Vector2i, goal: Vector2i, optimize: bool = true) -> Array:
	# Kolejki A* - open_set to węzły do przetworzenia
	var open_set = []
	var came_from = {}  # Śledzenie poprzedników

	# Koszty
	var g_score = {}
	var f_score = {}  # Resetowanie globalnej zmiennej
	...
	while open_set.size() > 0:
		# Sortowanie węzłów - użycie Callable bez dodatkowego argumentu
		open_set.sort_custom(Callable(self, "_compare_nodes"))

		var current = open_set.pop_front()

		# Jeśli dotarliśmy do celu, buduj ścieżkę
		if current == goal:
			var path = []
		...
		for neighbor in get_neighbors(current):
			var tentative_g_score = g_score.get(current, INF) + 1  # Koszt przejścia (stały: 1)

			if tentative_g_score < g_score.get(neighbor, INF):  # Lepsza ścieżka
	...
	return []  # Jeśli brak ścieżki, zwróć pustą listę

# Funkcja pomocnicza do porównywania węzłów
func _compare_nodes(a: Vector2i, b: Vector2i) -> int:
	var f_a = f_score.get(a, INF)
	var f_b = f_score.get(b, INF)
	return f_a - f_b  # Zwróć różnicę kosztów (zwraca int zgodnie z sort_custom)
...
func get_neighbors(node: Vector2i) -> Array:
	var neighbors = [
		node + Vector2i(1, 0),  # Prawy
		node + Vector2i(-1, 0),  # Lewy
		node + Vector2i(0, 1),  # Dół
		node + Vector2i(0, -1),  # Góra
		node + Vector2i(1, 1),   # Prawy-dół
		node + Vector2i(-1, -1), # Lewy-góra
		node + Vector2i(1, -1),  # Prawy-góra
		node + Vector2i(-1, 1)   # Lewy-dół
	]
	var valid_neighbors = []

	for neighbor in neighbors:
		if is_within_map(neighbor) and not is_obstacle(neighbor):
			valid_neighbors.append(neighbor)
	return valid_neighbors
		
func highlight_movement_options(unit: CharacterBody2D):
	clear_highlights()  # Usuń poprzednie podświetlenia

	var position = local_to_map(unit.position)
	for neighbor in get_neighbors(position):
		# Zapisz oryginalny typ kafelka
		if neighbor not in original_tiles:
			original_tiles[neighbor] = get_cell_atlas_coords(neighbor)

		var original_tile = original_tiles[neighbor]
		var highlight_tile = highlight_map.get(original_tile, Vector2i(0, 1))  # Domyślny podświetlacz (0,1)

		# Ustaw podświetlenie
		set_cell(neighbor, 0, highlight_tile)
		highlighted_tiles.append(neighbor)
		
func clear_highlights():
	for tile in highlighted_tiles:
		if tile in original_tiles:
			set_cell(tile, 0, original_tiles[tile])  # Przywróć oryginalny typ kafelka
